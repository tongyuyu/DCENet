...
class PGNet(nn.Module):
    def __init__(self, cfg=None):
        ...
        self.sqz_r5 = BasicConv2d(512, 64, 1)
        self.sqz_r4 = BasicConv2d(256, 64, 1)
        self.sqz_r3 = BasicConv2d(128, 64, 1)
        self.sqz_r2 = BasicConv2d(64, 64, 1)
    ...
    def forward(self, x,shape=None,mask=None):
        ...
        r5_1 = F.interpolate(r5_1, size=(62,62), mode='bilinear', align_corners=True) # 8 64 62 62
        r4_1 = F.interpolate(r4_1, size=(62,62), mode='bilinear', align_corners=True) # 8 64 62 62
        r3_1 = F.interpolate(r3_1, size=(124,124), mode='bilinear', align_corners=True) # 8 64 124 124
        r2_1 = F.interpolate(r2_1, size=(248,248), mode='bilinear', align_corners=True) # 8 64 248 248

        s34 = self.ALC1(s3_1, s4_1)  # 8 64 14 14
        s34up = self.upsample2(s34)  # 8 64 28 28

        s234 = self.ALC2(s2_1, s34up)  # 8 64 28 28
        s234up = self.upsample2(s234)  # 8 64 56 56
        s1234 = self.ALC3(s1_1, s234up)  # 8 64 56 56

        s1234 = F.interpolate(s1234, size=(62, 62), mode='bilinear', align_corners=True)  # 8 64 28 28
        sr15 = self.multihead_att_256(s1234, r5_1)  # 64 28 28
        sr15 = F.interpolate(sr15, size=(62, 62), mode='bilinear')  # 64 62 62

        r45 = self.ALC4(r4_1, sr15)  # 8 64 62 62
        # print('r45.size',r45.size())
        # print('r3_1.size',r3_1.size())
        r34 = self.ALC5(r3_1, self.upsample2(r45))  # 8 64 124 124
        r23 = self.ALC6(r2_1, self.upsample2(r34))  # 8 64 248 248
        # 全是64通道，信息可能有损耗
        # r23 = self.conv448_64(r23) # 8 64 248 248
