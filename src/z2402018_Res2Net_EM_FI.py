...
class FI(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv3 = BasicConv2d(out_channel, out_channel,kernel_size=3, stride=1,padding=1)
        self.conv4 = BasicConv2d(out_channel, out_channel,kernel_size=3, stride=1,padding=1)
        self.conv_out = BasicConv2d(out_channel*2, out_channel,kernel_size=3, stride=1,padding=1)

    def forward(self, x, xup):
        
        x1 = self.conv0(x)
        xup_1 = self.conv1(xup)
        
        x2 = self.CA(x1)
        xup_2 = self.CA2(xup_1)

        x3 = torch.mul(x1, x2)
        xup_3 = torch.mul(xup_1, xup_2)

        x4 = self.conv3(x3)
        xup_4 = self.conv4(xup_3)

        x5 = xup_4 + x4
        x4_m = 1 - x4
        xup_4_m = 1 - xup_4
        xup_5 = torch.mul(x4_m, xup_4_m)

        xup_5_m = 1 - xup_5
        # print('xup_5_m.size:', xup_5_m.size())

        x6 = torch.cat((x5, xup_5_m), 1)
...
class PGNet(nn.Module):
    def __init__(self, cfg=None):
        ...
        self.conv64_64 = BasicConv2d(64, 64, 1)
    ...
    def forward(self, x,shape=None,mask=None):
        ...
        x54 = self.FI5(x5_1, x4_1)# 8 128 62 62

        x54_up = self.upsample2(x54)  # 8 128 124 124
        x43 = self.FI4(x54_up,x3_1)
        x43_up = self.upsample2(x43)  # 8 192 248 248
        x32 = self.FI3(x43_up,x2_1) # 8 256 248 248
        # print('x32.size():', x32.size())
        x21 = self.FI2(x32,x1_1) # 8 320 248 248
        
        


        # 全是64通道，信息可能有损耗
        r23 = self.conv64_64(x21) # 8 64 248 248
